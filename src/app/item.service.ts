...
export class ItemService {
  ...
  editItem(id: number, barcode: string, name: string, description: string, type: string, room: string = 'bath'
    ...
    return this.http.put('http://127.0.0.1:8000/api/item/' + id, {
      ...
      room: room, status: status, annotation: annotation, image: image,
      lend: lend, manufactor: manufactor
    },
  ...
  deleteItem(id) {
    return this.http.delete('http://127.0.0.1:8000/api/item/' + id)
    .subscribe(
      data => console.log(data)
    );
  }

  createItem(barcode: string, name: string, description: string, type: string, room: string = 'bath'
    , status: string, annotation: string, image: string, lend: number, manufactor: string) {

    return this.http.post('http://127.0.0.1:8000/api/item', {
      barcode: barcode, name: name, description: description, type: type,
      room: room, status: status, annotation: annotation, image: image,
      lend: lend, manufactor: manufactor
    },
